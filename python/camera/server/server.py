...
def create_app(server_type, conf, conf_path, camera_stream=None):
    ...
    def run_ansible_script():
        r = ansible_runner.run(private_data_dir='./ansible', playbook = 'config_hardware.yml', forks=10, status_handler = my_status_handler)
        r = ansible_runner.run(private_data_dir='./ansible', playbook = 'install_software.yml', forks = 10, status_handler = my_status_handler)
        r = ansible_runner.run(private_data_dir='./ansible', playbook = 'sync_time.yml', forks = 10, status_handler = my_status_handler)
        r = ansible_runner.run(private_data_dir='./ansible', playbook = 'reboot.yml', forks = 10, cmdline = '-- tags reboot', status_handler = my_status_handler)
        r = ansible_runner.run(private_data_dir='./ansible', playbook = 'sync_code.yml', forks = 10, status_handler = my_status_handler)
    ...
    def run_ansible_script_to_synchronise_clocks():
        r = ansible_runner.run(private_data_dir='./ansible', playbook = 'sync_time.yml', forks = 10, cmdline = '--tags experiment', status_handler = my_status_handler)
